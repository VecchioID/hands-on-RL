...
def train_worker(env, master, rank, lock, batch_size=64, num_epochs=100, start_epoch=0, max_step=200, render=False):
    ...
    for epoch_idx in range(start_epoch, start_epoch + num_epochs):
        ...
        for step_idx in range(max_step):
            ...
            worker.buffer.masks.append(not (terminated))
            ...
            if terminated:
                ...
                print(
                    f'{worker.name} | epoch {epoch_idx:3d} | cumulative reward (max): {cumulative_rewards[-1]:4.1f} ' +
...
def train(env, master, num_processes=4, batch_size=64, num_epochs=10, start_epoch=0, max_step=200, render=False):
    ...
    for rank in range(num_processes):
        p = mp.Process(target=train_worker,
                       args=(env, master, rank, lock, batch_size, num_epochs, start_epoch, max_step, render))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
if __name__ == '__main__':
    ...
    evaluate(env, master, checkpoint_path, num_epochs=10, max_step=max_step, render=False)